...
type TestValue = TestPrimitive | TestMap | TestValue[];
interface TestMap {
  [key: string]: TestValue;
}
interface TestWhere extends TestMap {
  tenantId?: string;
}
interface TestData extends TestMap {
  tenantId?: string;
}
interface TestArgs extends TestMap {
  where?: TestWhere;
  data?: TestData | TestData[];
}

const ensureArgs = (args: TestArgs | null | undefined): TestArgs => args ?? {};
...
describe('Tenant Extension Logic', () => {
  ...
  describe('addTenantFilter', () => {
    ...
    const addTenantFilter = (model: string, args: TestArgs | null): TestArgs | null => {
    ...
    it('should handle null args', () => {
      runWithTenant('100001', false, () => {
        const result = addTenantFilter('SysUser', null) as TestArgs;
        expect(result.where.tenantId).toBe('100001');
      });
    });
  });
  ...
  describe('setTenantId for create', () => {
    ...
    const setTenantId = (model: string, args: TestArgs | null): TestArgs | null => {
      if (!hasTenantField(model)) {
        return args;
      }
      ...
      const nextArgs = ensureArgs(args);
      const data = Array.isArray(nextArgs.data) ? null : (nextArgs.data ?? {});
  ...
  describe('setTenantIdForMany for createMany', () => {
    ...
    const setTenantIdForMany = (model: string, args: TestArgs | null): TestArgs | null => {
      if (!hasTenantField(model)) {
        return args;
      }
      ...
      if (Array.isArray(nextArgs.data)) {
        nextArgs.data = nextArgs.data.map((item: TestData) => ({
          ...item,
...
describe('TenantContext', () => {
  ...
  describe('concurrent async isolation', () => {
    it('parallel async operations under different tenant contexts do not bleed', async () => {
    ...
    it('outer context is restored after inner async run completes', async () => {
    ...
    it('setTenantId in one concurrent branch does not affect another', async () => {
      const [idA, idB] = await Promise.all([
        TenantContext.run({ tenantId: 'A-initial' }, async () => {
          await new Promise((resolve) => setTimeout(resolve, 8));
          TenantContext.setTenantId('A-mutated');
          await new Promise((resolve) => setTimeout(resolve, 2));
          return TenantContext.getTenantId();
        }),
        TenantContext.run({ tenantId: 'B-initial' }, async () => {
          await new Promise((resolve) => setTimeout(resolve, 5));
          return TenantContext.getTenantId();
        }),
      ]);
      expect(idA).toBe('A-mutated');
      expect(idB).toBe('B-initial');
    });
  });
});
