...
import { clipSysOperLogVarchar } from './oper-log-field.util';
...
export class OperlogService {
  ...
  async findAll(query: QueryOperLogDto) {
    ...
    const scopedWhere = this.tenantHelper.readWhereForDelegate(
      'sysOperLog',
      where as object,
    ) as Prisma.SysOperLogWhereInput;
    ...
    const [list, total] = await this.prisma.$transaction([
      ...
      this.prisma.sysOperLog.count({ where: scopedWhere }),
    ]);
  ...
  async logAction({
    ...
    params.operParam = clipSysOperLogVarchar(params.operParam);
    params.jsonResult = clipSysOperLogVarchar(params.jsonResult);
    params.errorMsg = clipSysOperLogVarchar(params.errorMsg);
