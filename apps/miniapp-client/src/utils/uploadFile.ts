...
export const uploadFileUrl = {
  /** C 端通用上传（需登录，multipart 字段名 `file`），见 `api/client-upload.ts` */
  CLIENT_UPLOAD_FILE: `${import.meta.env.VITE_SERVER_BASEURL}/client/upload/file`,
};
...
export function useUpload<T = string>(
  ...
  const startUpload = (tempFilePath: string) => {
    ...
    const uploadOptions: UploadFileOptions<T> = {
      url,
      tempFilePath,
      formData,
      data,
    ...
    if (onSuccess) uploadOptions.onSuccess = onSuccess;
    if (onError) uploadOptions.onError = onError;
    if (onComplete) uploadOptions.onComplete = onComplete;

    uploadFile<T>(uploadOptions);
  };
