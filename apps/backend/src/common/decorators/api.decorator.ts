...
type ApiEnumValue = string | number;
type ApiExampleValue = ApiPrimitive | ApiExampleObject | ApiExampleValue[];
interface ApiExampleObject {
  [key: string]: ApiExampleValue;
}
type ApiModelType = Type<object>;
...
export interface ApiOptions {
   ...
   * 请求示例
   */
  requestExample?: ApiExampleValue;

  /**
   * 响应示例
   */
  responseExample?: ApiExampleValue;
 ...
 */
function buildDataSchema(type?: ApiModelType, isArray?: boolean, isPager?: boolean) {
  // 无类型时返回简单成功标识
  if (!type) {
    return {
      type: 'object',
...
export const Api = (options: ApiOptions) => {
  ...
  if (fileUpload) {
    ...
    // 4. ApiBody - 请求体（如有）
    const bodyOptions = requestExample
      ? {
          type: body,
          required: true,
          examples: {
  ...
  // 11. 收集需要额外注册的模型
  const extraModels: ApiModelType[] = [Result];
 ...
 * 保留用于向后兼容
 */
export const ApiDataResponse = <TModel extends ApiModelType>(model?: TModel, isArray?: boolean, isPager?: boolean) => {
  const extraModels: ApiModelType[] = [Result];
