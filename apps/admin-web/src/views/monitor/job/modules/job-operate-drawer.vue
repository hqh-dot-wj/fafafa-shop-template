...
import { fetchCreateJob, fetchUpdateJob } from '@/service/api/monitor/job';
import { useFormRules, useNaiveForm } from '@/hooks/common/form';
import { $t } from '@/locales';
import { canEditJobField, isCodeManagedJob } from './job-editability';
...
const isCodeManaged = computed(() => props.operateType === 'edit' && isCodeManagedJob(props.rowData));

function isFieldEditable(field: keyof Model) {
  if (props.operateType === 'add') {
    return true;
  }

  return canEditJobField(props.rowData, field);
}
...
function handleShowCron() {
  if (!isFieldEditable('cronExpression')) {
    return;
  }
...
<template>
  <NDrawer v-model:show="visible" display-directive="show" :width="600" class="max-w-90%">
    <NDrawerContent :title="title" :native-scrollbar="false" closable>
      <NSpin :show="loading">
        <NAlert v-if="isCodeManaged" type="info" class="mb-16px" title="代码托管任务">
          仅允许修改状态和备注，任务名称、分组、调用目标、Cron 与执行策略以代码定义为准。若定义有变更，请回到列表页执行“同步任务定义”。
        </NAlert>
        <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="100">
          <NFormItem label="任务名称" path="jobName">
            <NInput v-model:value="model.jobName" placeholder="请输入任务名称" :disabled="!isFieldEditable('jobName')" />
          </NFormItem>
          <NFormItem label="任务分组" path="jobGroup">
            <DictSelect
              ...
              placeholder="请选择任务分组"
              :disabled="!isFieldEditable('jobGroup')"
            />
          </NFormItem>
          <NFormItem label="调用方法" path="invokeTarget">
            ...
            <NInput
              v-model:value="model.invokeTarget"
              placeholder="请输入调用目标字符串"
              :disabled="!isFieldEditable('invokeTarget')"
            />
          </NFormItem>
          <NFormItem label="cron表达式" path="cronExpression">
            <NInputGroup>
              <NInput
                ...
                :disabled="!isFieldEditable('cronExpression')"
              />
              <NButton type="primary" :disabled="!isFieldEditable('cronExpression')" @click="handleShowCron">
                生成表达式
              </NButton>
            </NInputGroup>
          </NFormItem>
          <NFormItem v-if="operateType === 'edit'" label="状态" path="status">
            <DictRadio v-model:value="model.status" dict-code="sys_job_status" :disabled="!isFieldEditable('status')" />
          </NFormItem>
          ...
          <NFormItem label="执行策略" path="misfirePolicy">
            <NRadioGroup v-model:value="model.misfirePolicy" :disabled="!isFieldEditable('misfirePolicy')">
          ...
          <NFormItem label="是否并发" path="concurrent">
            <NRadioGroup v-model:value="model.concurrent" :disabled="!isFieldEditable('concurrent')">
