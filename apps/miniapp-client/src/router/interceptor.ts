...
export const navigateToInterceptor = {
  ...
  invoke({ url, query }: { url: string; query?: Record<string, string> }) {
    ...
    // pages.json 路径常为 `pages/...` 且无前导 `/`，若当相对路径解析会拼错（如 pages/me/me → /pages/index/pages/me/me）
    if (path && !path.startsWith('/') && path.startsWith('pages/')) {
      path = `/${path}`;
    }
    ...
    // 小程序不进入 H5 登录/重置页，认证统一交给全局授权弹窗。
    if (isMp && !LOGIN_PAGE_ENABLE_IN_MP && LOGIN_PAGE_LIST.includes(path)) {
      useAuthStore().requireAuth();
      return false;
    }

    // 小程序里面使用平台自带的登录，则不走下面的 H5 登录页重定向逻辑
