...
describe('System module services', () => {
  ...
  describe('DeptService', () => {
    ...
    beforeEach(() => {
      ...
      service = new DeptService(prisma, deptRepo as any, cls as any, tenantHelper);
    ...
    it('should create dept by inheriting parent ancestors', async () => {
      (prisma.sysDept.findFirst as jest.Mock).mockResolvedValue({ ancestors: '0' });
  ...
  describe('DictService', () => {
    ...
    beforeEach(() => {
      prisma = createPrismaMock();
      const tenantHelper = getTenantHelperTestProvider().useValue;
      service = new DictService(prisma, redisService as any, dictTypeRepo as any, dictDataRepo as any, tenantHelper);
      jest.clearAllMocks();
    });
  ...
  describe('NoticeService', () => {
    ...
    beforeEach(() => {
      service = new NoticeService(noticeRepo as any, notificationService as any);
      jest.clearAllMocks();
    });

    it('should publish enabled notices into the admin inbox', async () => {
      noticeRepo.create.mockResolvedValue({
        noticeId: 1,
        tenantId: '000000',
        noticeTitle: '系统维护',
        noticeType: '1',
      ...
      await service.create({ noticeTitle: '系统维护', noticeType: '1', status: Status.NORMAL } as any);
    ...
    it('should not publish stopped notices into the admin inbox', async () => {
      noticeRepo.create.mockResolvedValue({
        noticeId: 1,
        tenantId: '000000',
        noticeTitle: '草稿公告',
      ...
      await service.create({ noticeTitle: '草稿公告', noticeType: '2', status: Status.STOP } as any);

      expect(notificationService.send).not.toHaveBeenCalled();
    });

    it('should keep notice creation successful when inbox delivery fails', async () => {
      noticeRepo.create.mockResolvedValue({
        noticeId: 1,
        tenantId: '000000',
        noticeTitle: '系统公告',
      ...
      notificationService.send.mockRejectedValue(new Error('queue unavailable'));

      await expect(
        service.create({ noticeTitle: '系统公告', noticeType: '2', status: Status.NORMAL } as any),
      ).resolves.toMatchObject({
        code: 200,
      });
    });

    it('should publish notice when it changes from stopped to normal', async () => {
      noticeRepo.findById.mockResolvedValue({
        noticeId: 1,
        tenantId: '000000',
        noticeTitle: '系统公告',
        noticeType: '2',
      ...
      noticeRepo.update.mockResolvedValue({
        noticeId: 1,
        tenantId: '000000',
        noticeTitle: '系统公告',
        noticeType: '2',
      ...
      await service.update({ noticeId: 1, noticeTitle: '系统公告', noticeType: '2', status: Status.NORMAL } as any);
    ...
    it('should not republish notice when editing an already normal notice', async () => {
      noticeRepo.findById.mockResolvedValue({
        noticeId: 1,
        tenantId: '000000',
        noticeTitle: '系统公告',
      ...
      await service.update({ noticeId: 1, noticeTitle: '系统公告更新', noticeType: '2', status: Status.NORMAL } as any);
  ...
  describe('PostService', () => {
    ...
    beforeEach(() => {
      ...
      service = new PostService(deptService, postRepo as any);
      jest.clearAllMocks();
    });
  ...
  describe('RoleService', () => {
    ...
    beforeEach(() => {
      prisma = createPrismaMock();
      const tenantHelper = { readWhereForDelegate: (_d: string, w?: object) => ({ ...(w ?? {}) }) };
      service = new RoleService(prisma, roleRepo as any, menuService as any, tenantHelper as any);
    });
    ...
    it('should merge permissions from menu service', async () => {
      (prisma.sysRoleMenu.findMany as jest.Mock).mockResolvedValue([{ menuId: 10 }]);
