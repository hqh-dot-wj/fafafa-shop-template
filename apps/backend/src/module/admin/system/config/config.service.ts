...
export class ConfigService {
   ...
   */
  @Transactional()
  async updateByKey(dto: UpdateConfigByKeyDto) {
    const config = await this.configRepo.findByConfigKey(dto.configKey);
    BusinessException.throwIfNull(config, '参数不存在', ResponseCode.DATA_NOT_FOUND);
    await this.configRepo.update(config.configId, {
      configValue: dto.configValue,
    });
    await this.redisService.del(`${CacheEnum.SYS_CONFIG_KEY}${dto.configKey}`);
    return Result.ok();
  }
