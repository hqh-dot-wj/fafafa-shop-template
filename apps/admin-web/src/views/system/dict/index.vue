...
import DictImportModal from './modules/dict-import-modal.vue';
import DictSortModal from './modules/dict-sort-modal.vue';
import DictStatsModal from './modules/dict-stats-modal.vue';
...
const { bool: importModalVisible, setTrue: openImportModal } = useBoolean();
const { bool: statsModalVisible, setTrue: openStatsModal } = useBoolean();
const { bool: sortModalVisible, setTrue: openSortModal } = useBoolean();
...
function handleOpenSortModal() {
  if (!searchParams.dictType) {
    window.$message?.warning('请先在左侧选择字典类型');
    return;
  }
  openSortModal();
}
    ...
    <div class="h-full flex-col-stretch gap-12px overflow-hidden lt-sm:overflow-auto">
      ...
      <NCard :title="() => tableTitle" :bordered="false" size="small" class="card-wrapper sm:flex-1-hidden">
        <template #header-extra>
          ...
          >
            <template #prefix>
              <NButton v-if="hasAuth('system:dict:add')" ghost size="small" @click="openImportModal">
                <template #icon>
                  <icon-material-symbols:upload-rounded class="text-icon" />
                </template>
                导入
              </NButton>
              <NButton v-if="hasAuth('system:dict:list')" ghost size="small" @click="openStatsModal">
                <template #icon>
                  <icon-material-symbols:query-stats-rounded class="text-icon" />
                </template>
                统计
              </NButton>
              <NButton
                ...
                ghost
                size="small"
                :disabled="!searchParams.dictType"
                @click="handleOpenSortModal"
              >
                <template #icon>
                  <icon-material-symbols:swap-vert-rounded class="text-icon" />
                </template>
                排序
              </NButton>
        ...
        />
        <DictImportModal v-model:visible="importModalVisible" @submitted="getTreeData" />
        <DictStatsModal v-model:visible="statsModalVisible" />
        <DictSortModal
