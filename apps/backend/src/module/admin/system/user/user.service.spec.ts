...
describe('UserService', () => {
  ...
  describe('remove', () => {
    it('should not allow deleting system user', async () => {
      const result = await service.remove([1, 2]);

      expect(result.code).toBe(ResponseCode.BUSINESS_ERROR);
      expect(result.msg).toContain('系统用户不可删除');
      expect(userRepo.softDeleteBatch).not.toHaveBeenCalled();
    });
    ...
    it('should treat repeated soft delete as an idempotent no-op', async () => {
      (userRepo.softDeleteBatch as jest.Mock).mockResolvedValueOnce(0);

      const result = await service.remove([2]);

      expect(result.code).toBe(200);
      expect(result.data.count).toBe(0);
      expect(userRepo.softDeleteBatch).toHaveBeenCalledWith([2]);
    });
  });
