    ...
    >
      ...
      <NSpace vertical :size="12" class="mb-8px">
        <div class="mx-6px mb-8px flex-y-center justify-between">
          ...
          <NA type="primary" class="text-16px" @click="toggleLoginModule('reset-pwd')">
...
-->
    ...
    <div class="mt-24px w-full flex flex-col items-center gap-12px text-center text-16px text-#858585">
      <div class="flex flex-wrap justify-center gap-x-12px gap-y-8px">
        <NA type="primary" class="text-16px" @click="toggleLoginModule('code-login')">短信登录</NA>
        <span class="opacity-40">|</span>
        <NA type="primary" class="text-16px" @click="toggleLoginModule('reset-pwd')">忘记密码</NA>
        <span class="opacity-40">|</span>
        <NA type="primary" class="text-16px" @click="toggleLoginModule('bind-wechat')">微信绑定</NA>
      </div>
      <p class="m-0 max-w-360px text-14px leading-relaxed opacity-80">
        管理员账号由管理员分配，不提供公开自助注册。需要账号请联系运维或租户管理员。
      </p>
    </div>
  </div>
</template>
