...
> {
  ...
  async findPageWithFilter(
    ...
    const scopedWhere = this.scopeReadWhere(where as object) as Prisma.SysOperLogWhereInput;
  ...
  async countByBusinessType(businessType: number): Promise<number> {
    return this.delegate.count({
      where: this.scopeReadWhere({ businessType }) as Prisma.SysOperLogWhereInput,
    });
  }
  ...
  async countByOperName(operName: string): Promise<number> {
    return this.delegate.count({
      where: this.scopeReadWhere({ operName }) as Prisma.SysOperLogWhereInput,
    });
  }
  ...
  async findRecentLogs(limit: number = 10): Promise<SysOperLog[]> {
    return this.delegate.findMany({
      where: this.scopeReadWhere({}) as Prisma.SysOperLogWhereInput,
      take: limit,
