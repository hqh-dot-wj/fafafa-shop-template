...
演示用户与「演示只读」角色由 **Prisma seed** 写入（见 `apps/backend/prisma/seeds/00-platform/sys-menu-and-role-menu.ts` 等）。在仓库根或 `apps/backend` 下执行：
...
（等价：`pnpm exec prisma db seed`，具体入口见 `apps/backend/package.json` 的 `seed` 脚本。）

种子会按当前仓库定义创建/更新平台菜单、角色、演示账号等；**不要**再使用已不存在的 `scripts/init-demo-account.ts`。
...
```bash
# 1. 测试演示账户登录（全局前缀默认 /api，认证路由为 /auth/login）
curl -X POST http://localhost:8080/api/auth/login \
  -H "Content-Type: application/json" \
  -H "tenant-id: 000000" \
  -d '{"username":"demo","password":"demo123"}'

# 成功时返回体为统一 Result 包装，data 内含 access_token（见 LoginTokenVo）
```
...
默认访问 http://localhost:9527（见 `apps/admin-web/vite.config.ts`），你会看到：
...
`apps/admin-web/src/views/_builtin/login/modules/pwd-login.vue`
