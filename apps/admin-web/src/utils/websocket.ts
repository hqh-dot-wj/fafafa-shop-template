...
export function initWebSocket(url: any) {
  ...
  const fullSocketUrl = `${url}?Authorization=Bearer ${token}&clientid=${import.meta.env.VITE_APP_CLIENT_ID}`;
  if (
    activeSocketUrl === fullSocketUrl &&
    websocket &&
    [WebSocket.CONNECTING, WebSocket.OPEN].includes(websocket.readyState)
  ) {
    return websocket;
  }
  ...
  activeSocketUrl = fullSocketUrl;
  // 初始化 websocket
  websocket = new WebSocket(fullSocketUrl);
  websocketonopen();
  websocketonmessage();
  websocketonerror();
  websocketclose();
