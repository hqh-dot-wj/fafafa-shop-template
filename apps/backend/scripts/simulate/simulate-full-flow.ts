...
async function bootstrap() {
  ...
  try {
    ...
    console.log(
      fs
        .readFileSync(path.resolve(__dirname, '../src/module/client/order/order.service.ts'), 'utf8')
        .substring(10000, 15000),
    );
  ...
  const logCheck = (name: string, passed: boolean, msg?: string, data?: any) => {
    ...
    if (passed && data) {
      ...
      reportLines.push(JSON.stringify(data, (key, value) => (typeof value === 'bigint' ? value.toString() : value), 2));
    }
  };
  ...
  try {
    ...
    try {
      ...
      logCheck('6. Self-Purchase (Referrer)', true, 'Ref=Self Order created (No Commission)', {
    ...
    try {
      ...
      logCheck(
        '14. Refund Settled',
        comms.every((c) => c.status === 'CANCELLED') && wL1?.balance.toNumber() === 94,
        'Commission Cancelled & Wallet Deducted',
        {
    ...
    try {
      ...
      logCheck('15. Abnormal Referrer', true, 'Order created (Commission Generated despite Disabled status)', {
        commissions: comms,
      });
    } catch (e: any) {
