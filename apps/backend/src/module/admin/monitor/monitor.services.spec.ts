...
const tenantHelperTestMock = { readWhereForDelegate: (_d: string, w?: object) => ({ ...(w ?? {}) }) };
...
describe('Monitor module services', () => {
  ...
  describe('JobLogService', () => {
    ...
    beforeEach(() => {
      ...
      service = new JobLogService(prisma, tenantHelperTestMock as never);
    });
  ...
  describe('JobService', () => {
    ...
    const jobDefinitionSyncService = {
      syncOnStartup: jest.fn().mockResolvedValue(undefined),
      syncDefinitions: jest.fn().mockResolvedValue({}),
      hasDefinitionDrift: jest.fn().mockReturnValue(false),
    };
    const redisService = {
    ...
    beforeEach(() => {
      ...
      service = new JobService(
        ...
        redisService as any,
        tenantHelper as any,
        jobDefinitionSyncService as any,
      );
    });
  ...
  describe('LoginlogService', () => {
    ...
    beforeEach(() => {
      prisma = createPrismaMock();
      service = new LoginlogService(prisma, tenantHelperTestMock as never);
    });
  ...
  describe('OnlineService', () => {
    ...
    const service = new OnlineService(sessionService as any);
    ...
    it('should build online list from redis sessions', async () => {
      sessionService.getOnlineUsers.mockResolvedValue({
        list: [
          {
            tokenId: '1',
            user: { deptName: '研发部' },
    ...
    it('should delete session token from redis', async () => {
      ...
      expect(sessionService.deleteSession).toHaveBeenCalledWith('abc');
    });
  });
