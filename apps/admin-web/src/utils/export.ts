...
function getTableValue<T>(
  ...
  const dictMap = dicts as Partial<Record<string, string>> | undefined;
  ...
  if (isNotNull(dictMap?.[normalizedKey]) && isNotNull(row[normalizedKey])) {
    return $t(row[normalizedKey] as App.I18n.I18nKey);
  }

  return row[normalizedKey] ?? null;
}
