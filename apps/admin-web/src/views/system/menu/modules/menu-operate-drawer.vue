...
function handleMenuQueryParamParseError(error: unknown, mode: 'internal' | 'iframe') {
  reportActionError(error, {
    module: 'system-menu',
    operationCode: 'menu.form.init',
    stepCode: `menu.form.init.${mode}.queryParam`,
    stepName: mode === 'internal' ? '解析菜单内部链接参数' : '解析菜单 iframe 参数',
    metadata: {
      menuId: props.rowData?.menuId,
      isFrame: model.isFrame,
      queryParam: model.queryParam,
    },
  });
  window.$message?.error($t('page.system.menu.form.path.invalid'));
}
  ...
  if (props.operateType === 'edit' && props.rowData) {
    ...
    if (model.isFrame === '1') {
      try {
        ...
        handleMenuQueryParamParseError(error, 'internal');
        model.queryParam = '';
      }
      return;
    }
    ...
    try {
      ...
      handleMenuQueryParamParseError(error, 'iframe');
      model.queryParam = '';
    }
  }
}
      ...
      <NForm ref="formRef" :model="model" :rules="rules">
        <NGrid responsive="screen" item-responsive>
          ...
          <NFormItemGi v-if="!isBtn" span="12" :label="$t('page.system.menu.iconType')">
            <NRadioGroup v-model:value="iconType" class="radio-group-row">
          ...
          <NFormItemGi v-if="!isBtn" :span="12" path="isFrame">
            ...
            <NRadioGroup v-model:value="model.isFrame" class="radio-group-row">
              <NRadio
          ...
          <NFormItemGi v-if="isMenu" :span="12" path="isCache">
            ...
            <NRadioGroup v-model:value="model.isCache" class="radio-group-row">
