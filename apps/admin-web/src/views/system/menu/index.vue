...
function renderPrefix({ option }: { option: TreeOption }) {
  const raw = option.icon as string | null | undefined;
  // 库内 icon 为空串时表示「无图标」；若用 icon || VITE_MENU_ICON，空串会误判为假值而套上全局默认图标
  if (raw === null || raw === undefined || String(raw).trim() === '') {
    return <span class="inline-block h-16px w-16px shrink-0" aria-hidden="true" />;
  }
  const s = String(raw).trim();
  const renderLocalIcon = s.startsWith('local-icon-');
  const icon = renderLocalIcon ? undefined : s;
  const localIcon = renderLocalIcon ? s.replace('local-icon-', 'menu-') : undefined;
...
<template>
  <TableSiderLayout default-expanded>
    ...
    <template #sider>
      <div class="menu-sider-root h-full min-h-0 flex flex-col gap-8px">
        ...
        <NSpin :show="loading" class="menu-sider-spin min-h-0 flex flex-col flex-1">
          <NTree
            ref="menuTreeRef"
            ...
            :pattern="name"
            class="menu-tree min-h-200px flex-1 overflow-auto py-3"
            key-field="menuId"
...
/* 侧栏树：占满卡片剩余高度，展开变长时在区域内滚动，白底与布局等高 */
:deep(.menu-sider-spin.n-spin) {
  display: flex;
...
:deep(.menu-sider-spin .n-spin-content) {
  display: flex;
  flex-direction: column;
  flex: 1;
  min-height: 0;
