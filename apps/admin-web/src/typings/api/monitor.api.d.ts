...
declare namespace Api {
  ...
  namespace Monitor {
    ...
    /** 任务来源 */
    type JobSourceType = 'MANUAL' | 'CODE_MANAGED';

    /** 编辑模式 */
    type JobEditableMode = 'full' | 'statusRemarkOnly';
    ...
    type Job = Common.CommonRecord<{
      ...
      sourceKey?: string;
      /** 编辑模式 */
      editableMode: JobEditableMode;
      /** 定义漂移 */
      definitionDrift: boolean;
      /** 定义已移除 */
      definitionRemoved: boolean;
      /** 执行保护模式 */
      guardMode?: 'self-managed' | 'platform-lock';
    }>;
    ...
    /** 错误事件 */
    type ErrorEvent = Common.CommonRecord<{
      id: string;
      app: string;
      env?: string;
      level: 'warn' | 'error' | 'fatal' | string;
      requestId?: string;
      traceId?: string;
      errorId: string;
      tenantId?: string;
    ...
    /** 错误事件搜索参数 */
    type ErrorEventSearchParams = CommonType.RecordNullable<
      Pick<
        ErrorEvent,
        'app' | 'level' | 'traceId' | 'requestId' | 'errorId' | 'operationCode' | 'stepCode' | 'errorCode' | 'tenantId'
      > &
        Api.Common.CommonSearchParams
    >;

    /** 错误事件列表 */
    type ErrorEventList = Api.Common.PaginatingQueryRecord<ErrorEvent>;

    /** 步骤事件 */
    type StepEvent = Common.CommonRecord<{
      id: string;
      app?: string;
      requestId?: string;
      traceId?: string;
    ...
    /** 步骤事件搜索参数 */
    type StepEventSearchParams = CommonType.RecordNullable<
      Pick<StepEvent, 'traceId' | 'errorId' | 'operationCode' | 'stepCode' | 'status' | 'tenantId'> &
        Api.Common.CommonSearchParams
    >;

    /** 步骤事件列表 */
    type StepEventList = Api.Common.PaginatingQueryRecord<StepEvent>;
  }
}
