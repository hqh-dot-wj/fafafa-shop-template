...
> {
  ...
  async findUserRoles(userId: number): Promise<SysRole[]> {
    return this.prisma.sysRole.findMany({
      ...
      } as object) as Prisma.SysRoleWhereInput,
    });
  }
  ...
  async findPageWithMenuCount(
    ...
    const scopedWhere = this.scopeReadWhere(where as object) as Prisma.SysRoleWhereInput;
    // 先查询角色列表
    const [roles, total] = await this.prisma.$transaction([
      ...
      this.prisma.sysRole.count({ where: scopedWhere }),
    ]);

    // 再查询每个角色的菜单数量（sys_role_menu 为纯关联表，无 delFlag/tenantId，不可用 scopeReadWhere）
    const roleIds = roles.map((role) => role.roleId);
    const menuCounts =
      roleIds.length === 0
        ? []
        : await this.prisma.sysRoleMenu.groupBy({
            by: ['roleId'],
