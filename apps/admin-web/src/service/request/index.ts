...
import { appendTraceToMessage, normalizeAppError, reportClientError } from './error-monitoring';
import type { RequestInstanceState } from './type';
import { getStoredTenantId, shouldRequireTenantHeader } from './tenant';
  ...
  },
  {
    async onRequest(config) {
      ...
      if (token && !isToken) {
        ...
        const tenantHeader = authStore.userInfo.user?.tenantId ?? getStoredTenantId();
        if (tenantHeader) {
          Object.assign(config.headers, { 'tenant-id': String(tenantHeader) });
        } else if (shouldRequireTenantHeader(config.url)) {
          throw new Error('当前登录租户缺失，请重新登录后重试');
        }
      }
        ...
        if (success) {
          ...
          if (tenantHeader) {
            Object.assign(response.config.headers, {
              'tenant-id': String(tenantHeader),
            });
          } else if (shouldRequireTenantHeader(response.config.url)) {
            throw new Error('当前登录租户缺失，请重新登录后重试');
          }
    ...
    transformBackendResponse(response) {
      ...
      const envelope = response.data as Record<string, unknown> | null | undefined;
      if (!envelope || typeof envelope !== 'object') {
        return envelope;
      }

      // 标准 { code, msg, data: { rows, total } }：必须优先取 data，避免顶层误带空 rows 时错拿列表
    ...
    onError(error) {
      ...
      const requestFailure = resolveRequestFailure(error, normalized);
      let { message } = requestFailure;
      const { backendErrorCode } = requestFailure;

      if (handleLogoutErrorCode(backendErrorCode)) {
        return;
      }

      // when the token is expired, refresh token and retry request, so no need to show error message
      if (isExpiredTokenErrorCode(backendErrorCode)) {
        return;
      }
      ...
      reportClientError(error, {
        ...
        stepCode: normalized.stepCode,
        stepName: normalized.stepName,
      });

      message = appendTraceToMessage(message, error);
      showErrorMsg(flatRequest.state, message);
    },
  },
);

function resolveRequestFailure(
  error: AxiosError<App.Service.Response>,
  normalized: ReturnType<typeof normalizeAppError>,
) {
  if (error.code === BACKEND_ERROR_CODE) {
    return {
      message: error.response?.data?.msg || normalized.message,
  ...
  if (error.response) {
    ...
    return {
      message: (respData?.msg as string) || (respData?.message as string) || normalized.message,
  ...
  return {
    ...
    backendErrorCode: normalized.errorCode,
  };
}

function handleLogoutErrorCode(backendErrorCode: string) {
  if (!isLogoutErrorCode(backendErrorCode)) return false;
...
function isLogoutErrorCode(backendErrorCode: string) {
  return getServiceCodes(
    import.meta.env.VITE_SERVICE_LOGOUT_CODES,
    import.meta.env.VITE_SERVICE_MODAL_LOGOUT_CODES,
  ).includes(backendErrorCode);
}

function isExpiredTokenErrorCode(backendErrorCode: string) {
  return getServiceCodes(import.meta.env.VITE_SERVICE_EXPIRED_TOKEN_CODES).includes(backendErrorCode);
}

function getServiceCodes(...values: Array<string | undefined>) {
  return values.flatMap((value) => value?.split(',') || []).filter((code) => code);
}
...
function handleRepeatSubmit(config: InternalAxiosRequestConfig) {
  ...
  if (!isRepeatSubmit && (config.method === 'post' || config.method === 'put')) {
    // FormData 无法被 JSON.stringify 区分，易与「同 URL 短间隔」误判为重复提交；上传类请显式 headers.repeatSubmit: false
    if (config.data instanceof FormData) {
      return;
    }
    const requestObj = {
...
async function handleEncrypt(config: InternalAxiosRequestConfig) {
  // 是否需要加密
  ...
  if (import.meta.env.VITE_APP_ENCRYPT === 'Y') {
    ...
    if (isEncrypt && (config.method === 'post' || config.method === 'put')) {
      ...
      // 使用 RSA 加密 AES 密钥
      const encryptedKey = await encrypt(encryptBase64(aesKey));
      // 使用 AES 加密请求数据
      const encryptedData =
