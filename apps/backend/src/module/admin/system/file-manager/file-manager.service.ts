...
import { OssStorageService } from 'src/module/admin/upload/services/oss-storage.service';
...
export class FileManagerService {
  ...
  async createFolder(createFolderDto: CreateFolderDto, username: string) {
    ...
    const exists = await this.prisma.sysFileFolder.findFirst({
      where: this.tenantHelper.readWhereForDelegate('sysFileFolder', {
        tenantId,
        parentId,
        folderName,
        delFlag: DelFlagEnum.NORMAL,
      }) as Prisma.SysFileFolderWhereInput,
    });
    ...
    let folderPath = '/';
    if (parentId > 0) {
      const parent = await this.prisma.sysFileFolder.findFirst({
        where: this.tenantHelper.readWhereForDelegate('sysFileFolder', {
          folderId: parentId,
          tenantId,
        }) as Prisma.SysFileFolderWhereInput,
  ...
  async updateFolder(updateFolderDto: UpdateFolderDto, username: string) {
    ...
    const folder = await this.prisma.sysFileFolder.findFirst({
      where: this.tenantHelper.readWhereForDelegate('sysFileFolder', {
        folderId,
        tenantId,
      }) as Prisma.SysFileFolderWhereInput,
  ...
  async deleteFolder(folderId: number, username: string) {
    ...
    if (!folder || folder.delFlag === DelFlagEnum.DELETE) {
      return Result.fail(ResponseCode.INTERNAL_SERVER_ERROR, '文件夹不存在');
    }
  ...
  async listFolders(query: ListFolderDto) {
    ...
    const folders = await this.prisma.sysFileFolder.findMany({
      where: this.tenantHelper.readWhereForDelegate('sysFileFolder', where as object) as Prisma.SysFileFolderWhereInput,
  ...
  /**
   * 列表接口：私有 Bucket 下 OSS 直链在浏览器匿名 GET 会 403，缩略图与预览需短时签名 URL
   */
  private applyOssSignedReadUrlsForList(rows: SysUpload[]): SysUpload[] {
    ...
    return rows.map((row) => {
      ...
      const mainKey = this.ossStorage.parseObjectKeyFromPublicUrl(row.url);
      if (!mainKey) {
        return row;
      }
      const signedMain = this.ossStorage.signGetObjectUrl(mainKey, expires);
      if (!signedMain) {
        return row;
      }
      let nextThumbnail = row.thumbnail;
      if (nextThumbnail) {
        const tk = this.ossStorage.parseObjectKeyFromPublicUrl(nextThumbnail);
        if (tk) {
          const st = this.ossStorage.signGetObjectUrl(tk, expires);
          if (st) {
            nextThumbnail = st;
          }
        }
      } else if (this.isLikelyImageExt(row.ext)) {
        nextThumbnail = signedMain;
      }
      return { ...row, url: signedMain, thumbnail: nextThumbnail };
    });
  }

  private isLikelyImageExt(ext: string | null | undefined): boolean {
    if (!ext) {
      return false;
    }
    const e = ext.toLowerCase().replace(/^\./u, '');
    return ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg', 'ico'].includes(e);
  }
  ...
  async listFiles(query: ListFileDto) {
    ...
    this.logger.debug('[文件查询] 查询条件:', JSON.stringify(scopedWhere, null, 2));
    ...
    const rowsForClient = this.applyOssSignedReadUrlsForList(rows);

    return Result.ok({ rows: rowsForClient, total });
  }
  ...
  async moveFiles(moveFileDto: MoveFileDto, username: string) {
    ...
    // 验证目标文件夹
    if (targetFolderId > 0) {
      const targetFolder = await this.prisma.sysFileFolder.findFirst({
        where: this.tenantHelper.readWhereForDelegate('sysFileFolder', {
      ...
      if (!targetFolder || targetFolder.delFlag === DelFlagEnum.DELETE) {
  ...
  async getShare(getShareDto: GetShareDto) {
    ...
    // 公开分享：仅用 shareId 定位，禁止 readWhereForDelegate（会合并当前请求租户，导致跨租户分享链接失效）
    const share = await this.prisma.sysFileShare.findFirst({
      where: { shareId },
    });
    ...
    // 文件按分享记录上的 tenantId 限定，禁止 readWhereForDelegate（避免与请求上下文租户合并冲突）
    const file = await this.prisma.sysUpload.findFirst({
      where: {
  ...
  async cancelShare(shareId: string, username: string) {
    ...
    const share = await this.prisma.sysFileShare.findFirst({
      where: this.tenantHelper.readWhereForDelegate('sysFileShare', {
        shareId,
        tenantId,
      }) as Prisma.SysFileShareWhereInput,
    });
  ...
  async getRecycleList(query: ListFileDto) {
    ...
    const [files, total] = await Promise.all([
      ...
      this.prisma.sysUpload.count({ where: scopedWhere }),
    ]);
  ...
  async getFileVersions(uploadId: string) {
    ...
    // 获取当前文件
    const currentFile = await this.prisma.sysUpload.findFirst({
      where: this.tenantHelper.readWhereForDelegate('sysUpload', {
        uploadId,
        tenantId,
      }) as Prisma.SysUploadWhereInput,
  ...
  async restoreVersion(fileId: string, targetVersionId: string, username: string) {
    ...
    // 获取目标版本文件
    const targetVersion = await this.prisma.sysUpload.findFirst({
      where: this.tenantHelper.readWhereForDelegate('sysUpload', {
        uploadId: targetVersionId,
        tenantId,
      }) as Prisma.SysUploadWhereInput,
    });

    if (!targetVersion || targetVersion.delFlag !== DelFlagEnum.NORMAL) {
  ...
  async downloadFile(uploadId: string, token: string, res: Response) {
    ...
    // 下载令牌已携带目标 tenantId，禁止 readWhereForDelegate（避免与请求上下文租户合并冲突）
    const file = await this.prisma.sysUpload.findFirst({
      where: {
        uploadId,
        tenantId,
    ...
    if (file.storageType === 'local') {
      ...
      // 云端对象：私有 Bucket 时直链 403，优先 302 到短时签名 URL
      const key = this.ossStorage.parseObjectKeyFromPublicUrl(file.url);
      if (key && this.ossStorage.isConfigured()) {
        const signed = this.ossStorage.signGetObjectUrl(key, 600);
        if (signed) {
