...
function getRouteRoles(route: ElegantConstRoute) {
  const routeRoles = route.meta?.roles;

  return Array.isArray(routeRoles) ? routeRoles.filter((role): role is string => typeof role === 'string') : [];
}
...
function getGlobalMenuByBaseRoute(route: RouteLocationNormalizedLoaded | ElegantConstRoute) {
  ...
  const { title, i18nKey, localIcon, iconFontSize } = meta;
  const noMenuIcon = meta.noMenuIcon === true;
  const resolvedIconSize = iconFontSize || 20;
  ...
  if (!noMenuIcon) {
    const icon = isNotNull(meta.icon) ? meta.icon : import.meta.env.VITE_MENU_ICON;
    menu.icon = SvgIconVNode({ icon, localIcon, fontSize: resolvedIconSize });
  } else {
    // 无图标时不省略 prefix 槽，避免 NMenu 与带 icon 的同级项相比文字整体左移（如营销管理与其它一级目录）
    menu.icon = () =>
      h('span', {
        class: 'inline-block shrink-0',
        style: { width: `${resolvedIconSize}px`, height: `${resolvedIconSize}px` },
        'aria-hidden': true,
      });
  }
