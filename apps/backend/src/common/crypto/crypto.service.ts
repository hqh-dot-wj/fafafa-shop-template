...
type CryptoPayload = object | string | number | boolean | null;
...
export class CryptoService implements OnModuleInit {
   ...
   * 3. 使用 AES 密钥解密数据
   */
  decryptRequest(encryptedKey: string, encryptedData: string): CryptoPayload {
    ...
    // 4. 解析 JSON
    return JSON.parse(decryptedJson) as CryptoPayload;
  }
   ...
   */
  encryptResponse(data: CryptoPayload, clientAesKey?: string): { encryptedKey: string; encryptedData: string } {
