...
module.exports = {
  ...
  prod: {
    ...
    runMigration: false, // 是否运行 prisma:deploy（migrate deploy + campaign cutover）
    runSeed: false, // 是否补导湖南种子（不 DROP 库；全量清库请 SSH 跑 scripts/prisma/remote-path-a-align.sh）
    dbPush: false, // 勿与 runMigration 同时开；Path A 对齐用 migrate deploy，不用 dbPush
