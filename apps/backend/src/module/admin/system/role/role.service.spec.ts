...
describe('RoleService', () => {
  ...
  describe('changeStatus', () => {
    it('should reject status changes for the super admin role', async () => {
      ...
      expect(result.msg).toContain('超级管理员角色不可停用');
      expect(prisma.sysRole.update).not.toHaveBeenCalled();
    });
  ...
  describe('remove', () => {
    it('should reject deleting the super admin role', async () => {
      const result = await service.remove([1, 2]);

      expect(result.code).toBe(ResponseCode.BUSINESS_ERROR);
      expect(result.msg).toContain('超级管理员角色不可删除');
      expect(prisma.sysRole.updateMany).not.toHaveBeenCalled();
    });
    ...
    it('should soft delete roles', async () => {
      ...
      expect(prisma.sysRole.updateMany).toHaveBeenCalledWith({
        where: { roleId: { in: [2, 3] }, delFlag: DelFlagEnum.NORMAL },
        data: { delFlag: DelFlagEnum.DELETE },
      });
    });

    it('should treat repeated soft delete as an idempotent no-op', async () => {
      ...
      expect(prisma.sysRole.updateMany).toHaveBeenCalledWith({
        where: { roleId: { in: [2] }, delFlag: DelFlagEnum.NORMAL },
        data: { delFlag: DelFlagEnum.DELETE },
      });
    });
  });
